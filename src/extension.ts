...
import * as fs from 'fs'; // Needed for path checks if workspace.fs isn't enough or for path math

// --- Global State ---
let currentPanel: vscode.WebviewPanel | undefined = undefined;
let lastImageUrl: string | undefined = undefined;
let activeImageTarget: { editor: vscode.TextEditor, line: number } | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('imageDecorator.showImage', async () => {
...
function handleEditorEvent(context: vscode.ExtensionContext, editor: vscode.TextEditor) {
...
function createOrUpdateWebview(context: vscode.ExtensionContext, imageUrl: string, editor: vscode.TextEditor, line: number) {
    // @ts-ignore
...
function setupPanelListeners(context: vscode.ExtensionContext, panel: vscode.WebviewPanel) {
    panel.onDidDispose(() => {
...
async function updateImageStyle(editor: vscode.TextEditor, lineIndex: number, style: any) {
    const document = editor.document;
...
async function updateInlineStyle(editor: vscode.TextEditor, line: vscode.TextLine, text: string, style: any) {
    const styleParts = [];
...
async function getLinkedCssFiles(editor: vscode.TextEditor): Promise<vscode.Uri[]> {
...
async function updateLinkedCSS(editor: vscode.TextEditor, selector: string, style: any, createIfMissing: boolean = false): Promise<boolean> {
...
function getWebviewContent(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, imageUrl: string) {
    const scriptUri = panel.webview.asWebviewUri(
        vscode.Uri.file(path.join(context.extensionPath, "src", "webview", "main.js"))
    );
